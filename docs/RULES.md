...
This reference lists every built-in static rule. Run `php artisan guard:explain LG-...` for module-specific impact, response guidance, analysis limitations, Laravel-version context, false-positive review steps, narrow suppression recipes, and structured vulnerable/safer code examples. The package-owned dashboard exposes the same guidance from each expandable rule row.
