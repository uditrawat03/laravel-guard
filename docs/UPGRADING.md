...
Newly written baselines use `schema: laravel-guard/baseline` and `schema_version: 4`. Schema 4 adds `acceptance.approvers` and enforceable entry-count, TTL, approval-count, expiration, reason, and severity policies. Schema 1-3 baselines remain readable, but a document that violates current policy is not used to suppress findings. Regenerate a legacy baseline to add current governance metadata.
